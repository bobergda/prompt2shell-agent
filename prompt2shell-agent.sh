...
if [ "$SHOW_HELP" -eq 1 ]; then
  cat <<'EOF'
Usage: ./prompt2shell-agent.sh [options] [-- app_args...]
...
if [ "$UPDATE_REQUIREMENTS" -eq 1 ]; then
  echo "[prompt2shell-agent] Upgrading core packages in virtualenv"
  python -m pip install --upgrade openai termcolor distro prompt_toolkit
  echo "[prompt2shell-agent] Writing pinned versions to requirements.txt"
...
with open(req_file, "w", encoding="utf-8") as handle:
    handle.write("\n".join(lines) + "\n")
print("[prompt2shell-agent] Updated:", ", ".join(lines))
PY
fi

if [ "$RUN_TESTS" -eq 1 ]; then
  echo "[prompt2shell-agent] Running tests"
